...
|---|---|---|---|
| INV-001 | `chart-quest.html` is the source of truth; `index.html` is its generated mirror and must not be hand-edited. | `docs/operations/README.md`; `docs/operations/GitWorkflow.md` | Confirmed on exact build-362 candidate `3ef4bf6`: source, root mirror, and site artifact are byte-identical at `d36bb51f...`. |
| INV-002 | A manual trade close is a distinct result from a stop-loss or target exit; replay/summary text must not characterize a manual close as a stop loss. | `CHARTQUEST_RC_RELEASE_2026-08-10.md`, P0b | Confirmed on exact build 362 through focused boundary/persistence coverage and Browser F3/F4. |
| INV-003 | Every resolved trade must reach the post-trade replay/summary path, including manual and hour-close paths. | `CHARTQUEST_RC_RELEASE_2026-08-10.md`, P0d | Confirmed for build-362 beta/Level-1 scope: genuine trade-three replay/details is required and fail-closed until intentional close; Browser F5 passed. L4+ forced-hour semantics remain a canonical-definition/post-beta boundary. |
| INV-004 | Trade-time encounters covered by the RC pass defer rather than delete journal pages and breakable boxes. | `CHARTQUEST_RC_RELEASE_2026-08-10.md`, P0c | Confirmed on exact build 362 by focused lifecycle coverage and Browser F7, including exactly-once post-trade rewards. |
