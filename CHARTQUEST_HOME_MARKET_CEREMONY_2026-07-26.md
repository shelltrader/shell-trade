...
**Date:** 2026-07-26 · **Branch:** `feature/home-market-ceremony` · **Commit:** `aa43833` (off `c0e819a`, build 298)
...
| beat | what happens |
|---|---|
| **choose** | six premium cards — float, glow, light-sweep, chart-style preview behind each, its own character line |
...
`HOME_MARKETS` drives the entire ceremony. XRP, NVIDIA, Gold, EUR/USD = one line there plus a
`FACTION_CONFIG` entry. No UI changes, no logic changes. Apple, Tesla and S&P 500 ship as the first
cosmetic-only markets (`symbol:null, cosmetic:true`).
...
3. **Six cards on a small phone** means some scrolling below the fold. It reads fine, but if you
   want all six visible at once on a 375px screen I'd shrink the cards and drop the character line
   to a single clause.
