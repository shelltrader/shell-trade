...
| 2 | **Boss roster, order & progression** | No adding/removing/reordering bosses, no HP/difficulty change, no re-using the legacy boss object | `openBoss`/`bossRound` (~11189); `bosses/`; [boss_canon.md](boss_canon.md) |
...
| 9 | **Trading system** (outcome, setups, entry/SL/TP, portal role) | No change to how outcomes are decided, how setups generate, or entry/stop/target/portal semantics — and any change must conform to the doctrine | `resolveTrade` 12554, `commitTrade` 11739; [trading_canon.md](trading_canon.md) |
