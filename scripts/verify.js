 ...
 *   22 Build-364 beta blockers + viewport/CQSAFE/RC invariants (behavioural owner tests and source contracts)
 *   23 Build-364 local browser harness safety/syntax/self-test
 */
const fs = require('fs');
const os = require('os');
...
function run() {
  ...
  // 22 — BUILD-364 BETA BLOCKERS + VIEWPORT/CQSAFE/RC INVARIANTS. The collectible, first-trade,
  ...
  // replay-close, and trade-focus contracts that a static syntax pass cannot distinguish.
  {
    try {
      ...
      add('22', 'Build-364 beta blockers + viewport/CQSAFE/RC invariants', suite.ok ? 'PASS' : 'FAIL',
        suite.ok
          ? suite.detail
          : `${suite.passed}/${suite.total} passed · ${first.name}: ${String(first.error && first.error.message || first.error).slice(0, 120)}`);
    } catch (e) {
      add('22', 'Build-364 beta blockers + viewport/CQSAFE/RC invariants', 'FAIL',
  ...
  // the actual 7-flow + 16-cell execution and visible evidence.
  {
    try {
      ...
      add('23', 'Build-364 local browser QA harness', failures.length ? 'FAIL' : 'PASS',
        failures.length ? failures.join(' · ') : detail.join(' · '));
    } catch (e) {
      add('23', 'Build-364 local browser QA harness', 'FAIL',
